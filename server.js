...
app.post("/parse", auth, async (req, res) => {
  try {
    ...
    // If AI returned exactly 00:00:00 but input has no explicit hour reference
    // (only a date word like "tomorrow"/"morgen") — treat as no-time → picker
    if (result.datetime && /T00:00:00/.test(result.datetime)) {
      const hasExplicitHour = /\d{1,2}[:\-\.]\d{2}|\d{1,2}h\d{2}|\b\d{1,2}\s*Uhr\b|\bat\s+\d|\balle\s+\d|\ba\s+las\s+\d|\bum\s+\d|(?:^|\s)à\s+\d|(?:^|\s)às\s+\d|\bam\b|\bpm\b|[ap]\.m\.|\b(eins|zwei|drei|vier|fünf|sechs|sieben|acht|neun|zehn|elf|zwölf)\s+Uhr\b|вечора|вечера|вечером|увечері|ввечері|ночи|ночі|вночі|ночью|утра|утром|ранку|вранці|зранку|дня|дні|днем|після\s+обіду|після\s+обеда|годин[иіу]?|morning|evening|night|afternoon|abends|nachts|morgens|\bsoir\b|\bmatin\b|\bnoche\b|\btarde\b|por\s+la\s+ma[nñ]ana|de\s+la\s+ma[nñ]ana|da\s+manh[ãa]|de\s+manh[ãa]|\bnoite\b|\brano\b|\bsera\b|\bmattina\b|wieczor|wieczorem|полдень|полночь|noon|midnight|mittag|mitternacht|midi|minuit|mediod[ií]a|medianoche|mezzogiorno|mezzanotte|meio-?dia|meia-?noite|południe|północ/i.test(normInputGlobal);

      if (!hasExplicitHour) {
        if (DEBUG) console.log(`[NO TIME] AI returned 00:00 with no explicit hour → picker for: "${input}"`);
        const taskText = cleanTaskText(removeTriggerWords(result.text || input));
        return res.json({ ok: true, text: taskText, datetime: '', source: 'unparsed' });
      }
    }
